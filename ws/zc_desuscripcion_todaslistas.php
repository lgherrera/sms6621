...
$listas     = array();
$p_la       = filter_input(INPUT_GET, 'la', FILTER_VALIDATE_INT); //numero corto
$p_provider = 'Zcontents';
$p_service  = 'suscripcion';
$p_medio    = 'SMS'; //filter_input(INPUT_GET, 'origen', FILTER_VALIDATE_INT); //origen
$p_msisdn   = filter_input(INPUT_GET, 'msisdn', FILTER_VALIDATE_INT);
loginfo('Desuscribir msisdn: ' . $p_msisdn);
loginfo('Desuscribir origen: ' . $p_medio);
...
foreach ($listas as $key => $value) {
    foreach ($value as $p_la => $p_code) {
        $urlDesus = 'http://www.m-gateway.com/ws/epcs_api_listas_snd_v2_off.php?la=' . $p_la . '&medio=' . $p_medio . '&provider=' . $p_provider . '&service=' . $p_service . '&code=' . $p_code . '&app_tag=SMS&produc=1';
